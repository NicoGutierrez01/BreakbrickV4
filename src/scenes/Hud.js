...
export class Hud extends Scene {
  ...
  create() {
    this.points_text = this.add.text(10, 10,  getPhrase('Puntos: '), {
      fontSize: "24px",
      color: "#ffffff",
    });
    ...
    this.remaining_time_text = this.add.text(
      ...
      10,
      getPhrase(`Restante:${this.remaining_time.toString().padStart(2, "0")}`),
      {
        fontSize: "24px",
        color: "#ffffff",
      }
  ...
  update_timeout(timeout) {
    this.remaining_time_text.setText(
      getPhrase(`Restante:${timeout.toString().padStart(2, "0")}`)
    );
  }
}